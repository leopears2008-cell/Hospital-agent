...
content = content.replace('import { TAMIL_NADU_HOSPITALS } from "./src/data/tamilNaduHospitals.ts";\n', '');
content = content.replace('import { MOCK_DOCTORS } from "./src/data/doctors.ts";\n', '');

const newGetContext = `async function getSimpleContext(queryText: string) {
...
content = content.replace(/function getSimpleContext[\s\S]*?(?=\n\/\/ Instantiate)/, newGetContext);
