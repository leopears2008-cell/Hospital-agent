    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 backdrop-blur-sm p-4 sm:p-6 overflow-y-auto">
      <div className="bg-white rounded-3xl max-w-2xl w-full flex flex-col max-h-[90vh] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95">
          ...
          <>
            ...
            <div className="p-4 bg-white border-t border-slate-100 flex justify-between shrink-0">
              ...
              {step < 5 ? (
                ...
                <button onClick={() => setShowConfirmDialog(true)} disabled={loading} className="bg-emerald-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-emerald-700 shadow-lg shadow-emerald-200 transition-all flex items-center gap-2 disabled:opacity-70">
        ...
        <ConfirmationDialog
          ...
          title="Finalize Appointment Booking"
          message={`Are you sure you want to book an appointment with ${selectedDoctor?.name || 'Doctor'} at ${hospital.name} on ${date} at ${time}?`}
